...
function validate(route) {
  log.info(LOG_PREFIX, 'validate route:', JSON.stringify(route));

  const { path } = route;
  log.verbose(LOG_PREFIX, 'match route path with regex', path, ROUTE_REGEX);

  if (path.match(ROUTE_REGEX)) {
    log.info(LOG_PREFIX, 'route valid', JSON.stringify(route));
  } else {
    log.error(LOG_PREFIX, 'route invalid', JSON.stringify(route));
