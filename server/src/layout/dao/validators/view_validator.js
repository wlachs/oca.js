...
function validate(view) {
  log.info(LOG_PREFIX, 'validate view:', JSON.stringify(view));
  ...
  if (templateSlots.every((slot) => validateSlotContentAssignment(slot, content))) {
    log.info(LOG_PREFIX, 'view valid', JSON.stringify(view));
  } else {
    log.error(LOG_PREFIX, 'view invalid', JSON.stringify(view));
